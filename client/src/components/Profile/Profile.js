...
function Profile() {
    ...
    useEffect(() => {
        ...
        if (!userId || !token) {
            ...
            axios.get(`http://3.14.142.122/savedflights/${userId}`, {
            ...
            axios.get(`http://3.14.142.122/savedhotels/${userId}`, {
                headers: { 'Authorization': token }
            })
            .then(response => {
    ...
    const handleDeleteFlight = async (flightId) => {
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`http://3.14.142.122/deleteflight/${flightId}`, {
                headers: { 'Authorization': token }
            });
    ...
    const handleDeleteHotel = async (hotelId) => {
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`http://3.14.142.122/deletehotel/${hotelId}`, {
                headers: { 'Authorization': token }
            });
    ...
    const handleBookFlight = () => {
        ...
        if (document.getElementById('flightBookingForm').checkValidity()) {
            ...
            axios.post('http://3.14.142.122/bookflight', bookingDetails, {
            headers: {
                Authorization: token,
            },
            })
